 ...
 * 出題は単元の全スキルから、苦手なスキルほど出やすいように重みづけして選ぶ。
...
export const BossBattleModule: React.FC<Props> = ({ onExit }) => {
  ...
  const unlocked = isThemeUnlocked(UNLOCK_THEME, ratio);
  ...
  if (phase === 'INTRO') {
    const req = THEME_UNLOCK[UNLOCK_THEME].label;
    return (
        ...
        <div className="max-w-2xl mx-auto px-4 py-6">
          ...
          {!unlocked && (
            <div className="bg-white/5 border border-white/10 rounded-[28px] p-8 text-center">
              ...
              <p className="text-white/60 font-bold text-sm">テーマ「スノーワールド」を 解放すると 挑戦できます（{req}）</p>
                ...
                return (
                  ...
                  >
                    ...
                    <div className="flex-1">
                      ...
                      <div className="text-xs text-white/70 font-bold mt-1">さいだい{c.questionCount}問・単元の全スキルから出題</div>
                    </div>
                  </button>
                );
              })}
  ...
  const renderAnswer = (tp: TestProblem) => {
    ...
    switch (tp.kind) {
      case 'kihon': return <KihonRound level={tp.level} problem={tp.p} focus="answer" onNext={() => {}} onResult={onResult} nextLabel="つぎへ" />;
      case 'times': return <TimesRound level={tp.level} problem={tp.p} focus="answer" onNext={() => {}} onResult={onResult} nextLabel="つぎへ" />;
      case 'compare': return <CompareRound level={tp.level} problem={tp.p} focus="answer" onNext={() => {}} onResult={onResult} nextLabel="つぎへ" />;
      case 'base': return <BaseRound level={tp.level} problem={tp.p} focus="answer" onNext={() => {}} onResult={onResult} nextLabel="つぎへ" />;
      case 'ratio': return <RatioCompareRound level={tp.level} problem={tp.p} focus="judge" onNext={() => {}} onResult={onResult} nextLabel="つぎへ" />;
      case 'word': return <WordRound level={tp.level} problem={tp.p} onNext={() => {}} onResult={onResult} nextLabel="つぎへ" />;
      case 'error': return <BaiErrorRound example={tp.p} startStage="judge" onNext={() => {}} onResult={onResult} nextLabel="つぎへ" />;
    }
  };
