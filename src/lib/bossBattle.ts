...
function needsWrittenCalc(entry: BossPoolEntry): boolean {
  return !entry.skillId.startsWith('kihon');
}
...
export function pickBossQuestions(tier: BossTier, mastery: Record<string, SkillStat>): BossQuestion[] {
  ...
  return order.map((entry) => {
    ...
    const timeLimitSec = Math.max(8, Math.round(entry.baseSec * config.timeMultiplier)) + (config.timed && needsWrittenCalc(entry) ? WRITTEN_CALC_EXTRA_SEC : 0);
